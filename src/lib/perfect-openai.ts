...
let currentRegion =
  process.env.AWS_REGION || BEDROCK_REGIONS[currentRegionIndex];
...
export async function generateEmbedding(text: string): Promise<any> {
  ...
  while (attempts < maxRetries) {
    try {
      console.log(
        `Generating AWS Bedrock Cohere embedding (${currentRegion})...`,
      );
      ...
      console.error(
        `AWS Bedrock embedding error in ${currentRegion}:`,
      ...
      throw new Error(
        `Failed to generate embedding in all regions: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
...
Please provide a detailed, accurate answer based on the context provided. Include specific details like driver names, team names, race results, dates, and statistics where relevant. If you're discussing race results, mention positions, points, and performance details.`,
      ...
      console.error(
        `AWS Bedrock Claude error in ${currentRegion}:`,
        error instanceof Error ? error.message : String(error),
      );
